package com.company.pay.wechat.sdk;
...
import com.company.pay.wechat.sdk.WXPayConstants.SignType;

@Service("weChatPay")
public class WXPay implements InitializingBean, DisposableBean {
	...
	public String getNotifyUrl(String notifyAction) {
		return config.getNotifyUrl() + notifyAction;
	}
	...
	public Map<String, String> fillSandboxRequestData(Map<String, String> reqData) throws Exception {
		reqData.put("appid", config.getAppID());
		reqData.put("mch_id", config.getMchID());
		reqData.put("nonce_str", WXPayUtil.generateUUID());
		if (SignType.MD5.equals(this.signType)) {
		...
		reqData.put("sign", WXPayUtil.generateSignature(reqData, this.initKey, this.signType));
		return reqData;
	}
	...
	public Map<String, String> processResponseXmlNoSignal(String xmlStr) throws Exception {
		String RETURN_CODE = "return_code";
		String return_code;
		Map<String, String> respData = WXPayUtil.xmlToMap(xmlStr);
		if (respData.containsKey(RETURN_CODE)) {
	...
	public Map<String, String> unifiedOrder(Map<String, String> reqData) throws Exception {
	 ...
	 public void doGetSandboxSignKey() throws Exception {
		 if (useSandbox) 
		 {
	       ...
	       String sandboxSign = this.requestWithoutCert("/sandboxnew/pay/getsignkey", this.fillSandboxRequestData(data), 10000, 10000);
	       Map<String, String> maps = processResponseXmlNoSignal(sandboxSign);
	       if(maps.containsKey("return_code"))
	       {
	    	   String return_code =maps.get("return_code");
	    	   if(return_code.compareToIgnoreCase(WXPayConstants.SUCCESS)==0)
	    	   {
	    		   if(maps.containsKey("sandbox_signkey"))
	    		   {
	    			   this.config.setKey(maps.get("sandbox_signkey"));
	    		   }
	    	   }
	       }
		 }
	...
	public Map<String, String> unifiedOrder(Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs)
		...
		String respXml = "";
		if (this.useSandbox) {
			reqData.put("total_fee", "301");
			this.doGetSandboxSignKey();
			respXml = this.requestWithoutCert(url, this.fillRequestData(reqData), connectTimeoutMs, readTimeoutMs);
		} else {
			respXml = this.requestWithCert(url, this.fillRequestData(reqData), connectTimeoutMs, readTimeoutMs);
		}
		// this.requestWithCert(urlSuffix, reqData, connectTimeoutMs, readTimeoutMs)
		return this.processResponseXml(respXml);
	}
